...
static inline
struct timespec*
CGWMicrosecondsToTimeSpec(
    CGWMicroseconds     usec,
...
static inline
void
CGWTimingIntervalSleep(
    CGWTimingInterval   *ti
)
{
    struct timespec     t;
    CGWMicroseconds     now = CGWMicrosecondsMakeNow();
    CGWMicroseconds     dt_sec = (ti->next_cycle - now);
    
    if ( dt_sec > 0.0 ) {
        struct timespec dt_req, dt_rem;
        
        CGWMicrosecondsToTimeSpec(dt_sec, &dt_req);
        while ( nanosleep(&dt_req, &dt_rem) == -1 ) {
            dt_req = dt_rem;
        }
    }
    ti->next_cycle = CGWMicrosecondsMakeNow() + ti->cycle_time;
}
