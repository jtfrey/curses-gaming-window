...
typedef struct {
    CGWTurtleEvent          base;               /*!< base event fields */
    float                   theta;              /*!< the proposed new orientation angle (in radians) of the turtle */
...
/**
 * Get the delay between pixel-generating events
 * For purely aesthetic purposes, a delay can be imposed to slow the turtle's
 * drawing progress.
 * @param Yertle            the CGWTurtle object
 * @return                  the number of seconds to delay
 */
double CGWTurtleStateGetDrawingDelay(CGWTurtleRef Yertle);

/**
 * Set the delay between pixel-generating events
 * For purely aesthetic purposes, a delay can be imposed to slow the turtle's
 * drawing progress.  The \p delay_in_seconds gets translated to a timespec,
 * so the value has resolution to 1 ns.
 * @param Yertle            the CGWTurtle object
 * @param delay_in_seconds  exactly what the name says
 */
void CGWTurtleStateSetDrawingDelay(CGWTurtleRef Yertle, double delay_in_seconds);
 ...
 * Any range-corrections are applied to \p theta.  This is a state change and not an
 * action -- so no drawing.
 * @param Yertle        the CGWTurtle object
...
/**
 * Get the orientation vector corresponding to the turtle's angle
 * @param Yertle        the CGWTurtle object
 */
CGWPoint2D CGWTurtleStateGetOrientationVector(CGWTurtleRef Yertle);

/**
 * Move the turtle relative to its current position
 * Given the current position and angle of \p Yertle, move the turtle
 * the distance \p dposition.  This is a state change and not an
 * action -- so no drawing.
 *
 ...
 * are applied.  This is a state change and not an action -- so no drawing.
 * @param Yertle        the CGWTurtle object
...
/**
 * Move the turtle an arbitrary distance
 * From its current position \p Yertle is moved a distance of dposition,
 * drawing pixels as it goes (if the stylus is on).
 * @param Yertle        the CGWTurtle object
 * @param dposition     the distance to move
 */
void CGWTurtleActionMove(CGWTurtleRef Yertle, float dposition);

/**
 * Move the turtle to an arbitrary position in turtle space
 * From its current position \p Yertle is moved to turtle-space coordinate
 * \p position, drawing pixels as it goes (if the stylus is on).
 * @param Yertle        the CGWTurtle object
...
/**
 * Move the turtle through a curved path
 * Imagine a circle is centered at the \p control_point.  The vector joining the
 * \p control_point to the current position of \p Yertle provides the radius and
 * initial point on that circle; the turtle is effectively tangent to the circle.
 * The turtle is then moved through \p phi radians of arc.
 * @param Yertle        the CGWTurtle object
 * @param control_point center of the circle to which the turtle is tangent
 * @param phi           radians of arc to curve through; negative values rotate
 *                      clockwise, positive counter-clockwise
 */
void CGWTurtleActionCurve(CGWTurtleRef Yertle, CGWPoint2D control_point, float phi);

/**
 * Move the turtle through a curved path while contracting the radius
 * Functions like \ref CGWTurtleActionCurve() but the radius of the circle
 * changes as the turtle moves.  For every 1 radian of arc, the radius shrinks
 * by the fraction \p r_multiplier_per_radian.
 * @param Yertle                the CGWTurtle object
 * @param control_point         center of the circle to which the turtle is tangent
 * @param phi                   radians of arc to curve through; negative values rotate
 *                              clockwise, positive counter-clockwise
 * @param dr_per_radian         contraction factor; positive values shrink the circle,
 *                              negative values expand the circle
 * @param is_proportional_to_r  if true, the contraction should be a fraction of
 *                              the current radius; if false, the contraction is
 *                              a constant value
 */
void CGWTurtleActionCurveAndContract(CGWTurtleRef Yertle, CGWPoint2D control_point, float phi, float dr_per_radian, bool is_proportional_to_r);
